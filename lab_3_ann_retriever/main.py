...
class Tokenizer:
    ...
    def tokenize(self, text: str) -> list[str] | None:
        ...
        if not isinstance(text, str):
            return None
        text = re.findall(r'[a-zа-яё]+', text.lower())
        return self._remove_stop_words(text)
    ...
    def tokenize_documents(self, documents: list[str]) -> list[list[str]] | None:
        ...
        In case of corrupt input arguments, None is returned.
        """
        if not isinstance(documents, list) or not (isinstance(document, str) for document in documents):
            return None
        if not documents:
            return None
    ...
    def _remove_stop_words(self, tokens: list[str]) -> list[str] | None:
        ...
        In case of corrupt input arguments, None is returned.
        """
        if not tokens or not isinstance(tokens, list) or not (isinstance(token, str) for token in tokens):
            return None
...
class BasicSearchEngine:
    ...
    def retrieve_relevant_documents(
        ...
        if knn is None or not knn or not all(isinstance(index, int) or isinstance(distance, float)
            ...
            return None
        relevant_docs = [(value, self._documents[index]) for index, value in knn if 0 <= index < len(self._documents)]
