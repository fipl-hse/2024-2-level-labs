...
class DocumentVectorDB:
    ...
    def put_corpus(self, corpus: Corpus) -> None:
        ...
        tokenized_docs = [self._tokenizer.tokenize(document) for document in corpus]
        tokenized_docs = [tok for tok in tokenized_docs if tok]
    ...
    def get_vectors(self, indices: list[int] | None = None) -> list[tuple[int, Vector]]:
        ...
        """
        if indices is None:
            return [(i, self.__vectors[i]) for i in range(len(self.__vectors))]
        
        return [(i, self.__vectors[i]) for i in indices if 0 <= i < len(self.__vectors)]
