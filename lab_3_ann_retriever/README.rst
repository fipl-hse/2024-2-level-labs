...
Для коллекции эти значения TBD, то получатся следующие ``TF-IDF``:
...
На рисунке подписано, какие по индексу документы олицетворяются каждым блоком.

.. figure:: ./assets/images/kdtree_example.png

    Пример дерева точек K-мерного пространства
...
.. figure:: ./assets/images/vector_space_division_example.png
    ...
    Пример деления двумерного пространства
...
Вычислять медианную точку будем так:
...
цвета) будет идти по значению оси "Вектор" — перпендикулярно ей.

Зелёная и красные точки оказываются в левом подпространстве, а фиолетовая и
оранжевая — в правом.
...
оранжевую точку, которая является листом.

Тогда итоговое дерево такого пространства будет выглядеть примерно так:

.. figure:: ./assets/images/2d_tree_example.png

    Пример дерева двумерного пространства

Шаг 6.1. Объявить сущность узла дерева
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Начнём построение дерева с узла. Реализуйте инициализацию класса
:py:class:`lab_3_ann_retriever.main.Node`, отвечающего за узел дерева.

Данный класс имеет следующие внутренние атрибуты:

-  ``self.vector`` — точка (вектор), находящаяся в узле;
-  ``self.left`` — узел, который находится в левой половине пространства,
    делимого текущим узлом;
-  ``self.right`` — узел, который находится в правой половине пространства,
    делимого текущим узлом;
...
Данный класс имеет следующий внутренний атрибут:

-   ``self._root`` — начальное состояние дерева, которое на момент инициализации
    содержит специальное значение ``None``, заглушку до непосредственного создания
    дерева.
...
отвечающий за деление пространства по точкам и сохранения
дерева в атрибут класса.

Создайте список, содержащий следующую информацию о состоянии пространства
в изначальный момент:

1.  Вектора для точек, находящихся в текущем пространстве, и их индексы;
...
2. Проверьте, что в текущем пространстве есть вектора

Если узлов нет, то ищите новые узлы в следующем пространстве.

3.  Определите ось деления на подпространства

Определите, по какой из осей (axis) координат производить деление на
...
такая длина у одного вектора коллекции. Тогда осью деления будет ось
номер `0`.

4.  Отсортируйте вектора по значениям полученной оси в порядке возрастания
...
Все точки правее медианной будут относиться к правому подпространству,
...
Если родителя у точки нет, то есть узел-родитель имеет номер `-1`,
...
для поиска ближайшей к точке запроса точки узла дерева. В рамках простого алгоритма Вам
...
2.  Проверьте, что узел не является листом

Если узел является листом, то мы нашли ответ. Посчитайте расстояние до листа с помощью функции
:py:func:`lab_3_ann_retriever.main.calculate_distance`,
...
Используем следующее правило:

Если :math:`q[axis] < v[axis]`, то левое подпространство ближайшее. Иначе — правое.

Добавьте в список существующих узлов пару ``(узел ближайшего подпространства,
текущая глубина + 1)``.

5. Повторяйте шаги 1-4, пока в списке существует пара узла и глубины и мы не достигли листа
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Реализуйте метод
:py:meth:`lab_3_ann_retriever.main.NaiveKDTree.query`,
отвечающий получение списка точек, которые вычислены как
...
который до этого объявлялся в классе родителя.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Улучшим алгоритм поиска документов. В Шаге 6 осуществлялся поиск
...
.. figure:: ./assets/images/search_in_2d_example.png
    ...
    Пример поиска в двумерном пространстве
...
точку, как ближайшую к запросу.

Но, как мы видим, оранжевая точка находится ближе, хоть и лежит в другом подпространстве.
Постараемся учесть это в новой реализации.
...
Используйте функцию :py:func:`lab_3_ann_retriever.main.calculate_distance`,
...
Тогда сохраните в список лучших ближайших соседей пару значений
...
Используем следующее правило: если :math:`q[axis] < v[axis]`, то левое подпространство ближайшее.
Иначе — правое. `axis` определяется по формуле оси из Шага 6.3.
...
6.  Добавьте в список текущих узлов дальний узел-ребёнок при условии
...
После выполнения метода у Вас будет список из k пар расстояний и узлов, отсортированный в
порядке возрастания расстояний.
...
ответственный за поиск релевантного документа по запросу с помощью улучшенного К-мерного дерева.

Это класс, который наследуется от класса
:py:class:`lab_3_ann_retriever.main.SearchEngine`, то есть
использует его атрибуты и методы, а также добавляет новые.

Данный класс использует атрибуты ``SearchEngine``, но переопределяет значение
атрибута ``self._tree`` на экземпляр класса ``KDTree``, который создаётся
при инициализации.
...
Здесь и в дальнейших шагах используйте библиотеку для работы с текстовом
форматом ``json`` для того, чтобы сохранять и загружать состояния классов.

Шаг 10.1 Сохранить разреженный вектор
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Как можно заметить, вектора, которые получаются с помощью метрики ``TF-IDF``,
преимущественно состоят из нулей. Такие вектора называются разреженными.
...
отвечающую за возвращение состояния в формат разреженного вектора.
...
Расширьте класс ``Vectorizer`` методом
...
было успешно cохранено.

Пример состояния сохранён в файле ``"./assets/states/vectorizer_state_example.json"``.
...
Данный метод заменяет заполнение экземпляра класса через метод
...
Помните, что вектора разреженные, поэтому их хранение нужно оптимизировать.
...
документы и их вектора можно найти по соответствующим ключам ``"documents"``
и ``"document_vectors"``.
...
1.  ``"vector" : неразреженный вектор``. Используйте функцию для
    :py:func:`lab_3_ann_retriever.main.save_vector` для представления
    вектора узла в неразреженном виде;

2.  ``"payload" : self.payload``;

3.  ``"left_node" : сохранённый узел``. Используйте метод
    :py:meth:`lab_3_ann_retriever.main.Node.save`
    для левого узла-ребёнка, если он существует, иначе используйте
    значение ``None``;

4.  ``"right_node" : сохранённый узел``. Используйте метод
    :py:meth:`lab_3_ann_retriever.main.Node.save`
...
загружающий состояние узла дерева рекурсивно.

Заполните атрибуты экземпляра класса соответствующими значениями из
состояния.

Не забудьте привести вектор в разреженный вид с помощью функции
...
возвращающий состояние дерева в виде словаря c ключом ``"root"``.
...
отвечающий за присваивание атрибуту корневого узла экземпляра класса
``Node`` и последующий вызов метода
:py:meth:`lab_3_ann_retriever.main.Node.load`.
...
Заполните значения ключей "documents" и "vector_documents" с помощью
...
4.  Заполнение атрибута ``self._tree`` через метод дерева
...
Сравните с теми результатами, которые получились ранее при поиске с
помощью ``BasicSearchEngine`` и ``SearchEngine``.
