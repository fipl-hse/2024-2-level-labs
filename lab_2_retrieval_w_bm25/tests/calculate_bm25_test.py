...
class CalculateBM25Test(unittest.TestCase):
    ...
    def test_calculate_bm25_new_document(self):
        ...
        """
        expected = {
            'need': 0.0, 'friend': 0.0, 'time': 0.0, 'tale': 0.0, 'new': 0.0, 'named': 0.0,
            'story': 0.0, 'opened': 0.0, 'cat': 0.0, 'lived': 0.0, 'output': 0.0, 'writing': 0.0,
            'across': 0.0, 'library': 0.0, 'loved': 0.0, 'completely': 0.0, 'stories': 0.0,
            'saw': 0.0, 'upon': 0.0, 'smart': 0.0, 'kind': 0.0, 'began': 0.0, 'particular': 0.0,
            'love': 0.0, 'format': 0.0, 'told': 0.0, 'must': 0.0, 'reading': 0.0, 'thought': 0.0,
            'beautiful': 0.0, 'ugly': 0.0, 'shop': 0.0, 'document': 0.0, 'coffee': 0.0,
            'ghost': 0.0, 'string': 0.0, 'street': 0.0, 'boy': 0.0, 'function': 0.0
        }
    ...
    def test_calculate_bm25_bad_input(self):
        ...
        for bad_float_input in bad_float_inputs:
            self.assertEqual(
                expected,
                calculate_bm25(self.vocabulary, self.tokenized_documents[9],
                               self.idf, k1=bad_float_input, b=0.75,
            ...
            self.assertEqual(
                expected,
                calculate_bm25(self.vocabulary, self.tokenized_documents[9],
                               self.idf, k1=1.5, b=bad_float_input,
                               avg_doc_len=self.avg_doc_len, doc_len=self.doc_len)
            )
        ...
        for bad_list_input in bad_list_inputs:
            self.assertEqual(
                expected,
                calculate_bm25(bad_list_input, self.tokenized_documents[9],
                               self.idf, k1=1.5, b=0.75,
                               avg_doc_len=self.avg_doc_len, doc_len=self.doc_len)
            )
            self.assertEqual(
                expected,
                calculate_bm25(self.vocabulary, bad_list_input,
                               self.idf, k1=1.5, b=0.75,
                               avg_doc_len=self.avg_doc_len, doc_len=self.doc_len)
            )
