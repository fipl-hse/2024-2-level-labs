...
:math:`IDF = ln(\frac{N - \sum^{m}_{j=1}D + 0.5}{\sum^{m}_{j=1}D + 0.5})`, где:

* :math:`N` - количество документов;
* :math:`\sum^{m}_{j=1}D` - количество документов, в которых содержится интересующее слово.
...
функцию :py:func:`lab_2_retrieval_w_bm25.main.save_index`. Пример корректного сохранения
вы можете найти в папке ``assets``.
...
и золотым стандартом ``[1, 7, 5, 0, 9, 2, 3, 4, 6, 8]``.
